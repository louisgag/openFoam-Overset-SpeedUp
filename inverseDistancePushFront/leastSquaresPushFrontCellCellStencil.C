...
#include "leastSquaresPushFrontCellCellStencil.H"
...
{
    defineTypeNameAndDebug(leastSquaresPushFront, 0);
    addToRunTimeSelectionTable(cellCellStencil, leastSquaresPushFront, mesh);
}
}

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::cellCellStencils::leastSquaresPushFront::stencilWeights
(
    const point& sample,
...
) const
{
    ...
    if (shortC)
    {
        ...
        if (debug)
        {
            Pout << "Matrix ill conditioned. Use straight injection from central donor." << endl;
        }
        weights = 0.0;
        weights[0] = 1.0;
    }
...
Foam::cellCellStencils::leastSquaresPushFront::leastSquaresPushFront
(
    const fvMesh& mesh,
