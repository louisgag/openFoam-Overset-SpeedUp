...
#include "inverseDistancePushFrontCellCellStencil.H"
...
{
    defineTypeNameAndDebug(inverseDistancePushFront, 0);
    addToRunTimeSelectionTable(cellCellStencil, inverseDistancePushFront, mesh);
}
}

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

Foam::label Foam::cellCellStencils::inverseDistancePushFront::index
(
...
Foam::labelVector Foam::cellCellStencils::inverseDistancePushFront::index3
(
    const labelVector& nDivs,
    const label boxI
)
...
Foam::point Foam::cellCellStencils::inverseDistancePushFront::position
(
    const boundBox& bb,
    const labelVector& nDivs,
    const label boxI
...
void Foam::cellCellStencils::inverseDistancePushFront::fill
(
    PackedList<2>& elems,
    const boundBox& bb,
    const labelVector& nDivs,
...
void Foam::cellCellStencils::inverseDistancePushFront::markBoundaries
(
    const fvMesh& mesh,
...
Foam::treeBoundBox Foam::cellCellStencils::inverseDistancePushFront::cellBb
(
...
bool Foam::cellCellStencils::inverseDistancePushFront::overlaps
(
    const boundBox& bb,
    const labelVector& nDivs,
...
void Foam::cellCellStencils::inverseDistancePushFront::markPatchesAsHoles
(
...
void Foam::cellCellStencils::inverseDistancePushFront::markDonors
(
...
) const
{
    ...
    // 1. do processor-local src/tgt overlap
    {
        ...
        waveMethodIters::calculate(tgtMesh, srcMesh, tgtToSrcAddr);
        forAll(tgtCellMap, tgtCelli)
        {
...
void Foam::cellCellStencils::inverseDistancePushFront::findHoles
(
    const globalIndex& globalCells,
    const fvMesh& mesh,
...
void Foam::cellCellStencils::inverseDistancePushFront::seedCell
(
    const label cellI,
...
void Foam::cellCellStencils::inverseDistancePushFront::walkFront
(
    const scalar layerRelax,
    const int nPushFront,
...
void Foam::cellCellStencils::inverseDistancePushFront::stencilWeights
(
    const point& sample,
...
void Foam::cellCellStencils::inverseDistancePushFront::createStencil
(
    const globalIndex& globalCells
)
{
...
Foam::cellCellStencils::inverseDistancePushFront::inverseDistancePushFront
(
    const fvMesh& mesh,
...
bool Foam::cellCellStencils::inverseDistancePushFront::update()
{
    scalar layerRelax(dict_.getOrDefault("layerRelax", 1.0));
    int nPushFront(dict_.getOrDefault("nPushFront", 0));
    ...
    // Determine zone meshes and bounding boxes
    {
        ...
        forAll(meshParts, zonei)
        {
            ...
            if (subMesh.nPoints() && globalPatchBb.containsAny(subMesh.points()))
            {
                procBb[Pstream::myProcNo()][zonei] =
                    treeBoundBox(subMesh.points());

                Info << "zonei, procBb  " << zonei << " " << procBb[Pstream::myProcNo()][zonei] << endl;
                Info << "zonei, patchBb  " << zonei << " " << patchBb[zonei][Pstream::myProcNo()] << endl;

                if (procBb[Pstream::myProcNo()][zonei].overlaps(patchBb[zonei][Pstream::myProcNo()]))
                {
                    if (debug)
                    {
                        Pout << "Overlap pracBb was: " << zonei << " procBb: " << procBb[Pstream::myProcNo()][zonei] << endl;
                    }
                    procBb[Pstream::myProcNo()][zonei].intersect(patchBb[zonei][Pstream::myProcNo()]);
                    if (debug)
                    {
                        Pout << "Overlap proc/patch, Zone: " << zonei << " new procBb: " << procBb[Pstream::myProcNo()][zonei] << endl;
                    }
                }
...
//    List<treeBoundBoxList> patchBb(nZones);
    ...
    bool usingVoxels(0);
    scalar voxelSize;
//    List<treeBoundBoxList> patchBbMesh(nZones); // using both global and mesh bb to allow compraision
    if (dict_.readIfPresent("voxelSize",voxelSize))
    {
	    usingVoxels = 1;
    }
	...
	{
		Info << "volxelSize: " << voxelSize << endl;
	  // calculate nDivs from size, per proccessor, per zone per component

        

        
        forAll(meshParts, zonei) // making bb same for all zones
	    {
            const fvMesh& subMesh = meshParts[zonei].subMesh();
		    labelVector v(1,1,1);
            //patchBb[zonei][Pstream::myProcNo()] = patchBb[prevZonei][Pstream::myProcNo()];
            //Pout << "final new patchBb: " << patchBb[zonei][Pstream::myProcNo()] << endl;
            if(subMesh.nPoints())
    		{
                // boundBox lbb(subMesh.points(),false); // orig from Nicolas
                doubleVector vScalar(patchBb[zonei][Pstream::myProcNo()].span()/voxelSize);
	    	    v = labelVector(ceil(vScalar.x()),ceil(vScalar.y()),ceil(vScalar.z()));
            }
            const labelVector& dim = mesh_.geometricD();
            //set divisons 1 for not used dimensions and increase from 0 divisions for too small dimensions
            forAll(dim, i)
            {
                if (dim[i] == -1 ||  v[i] == 0)
                {
            ...
            patchDivisions[zonei] = v;
            if (debug)
            {
                Pout << "Zone: " << zonei << " nDivs: " << v << endl;
            }
        }
    ...
    // Print a bit
    {
        ...
        if (debug)
	{
            Pout<< type() << " : detected " << nZones
                << " mesh regions" << endl;
            forAll(nCellsPerZone, zoneI)
            {
                Pout<< indent<< "zone:" << zoneI
                    << " nCells:" << nCellsPerZone[zoneI]
                    << "  voxels:" << patchDivisions[zoneI]
                    << " bb:" << patchBb[zoneI][Pstream::myProcNo()]
                    << endl;
    ...
    walkFront(layerRelax, nPushFront, allStencil, allCellTypes, allWeight);
